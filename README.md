...
- **EN / ES** — toggles the recognition language between English and Spanish (a desktop notification shows the new language). Also reflects the MIDI language toggle below.
...
There is no HTTP endpoint and no external hotkey daemon. On Linux, mute and language can additionally be toggled from a MIDI controller (see below); otherwise control is entirely through this window. On Linux it is a GTK3 window (`gui_linux.*`); on macOS an AppKit window (`gui_darwin.*`) that runs as an accessory (no Dock icon, no menu bar).

## MIDI mute/language toggle (Linux only)

If you have a MIDI keyboard or controller connected, one key can toggle mute and another can toggle language, without leaving the app you are dictating into — no keyboard focus change, no mouse. It reads the ALSA rawmidi device directly, so there is nothing extra to install, and it works the same under X11 and Wayland. It is off unless you enable it.
...
VTT_MIDI_NOTE=48          # toggles mute
VTT_MIDI_LANG_NOTE=60     # toggles language; 60 = middle C (Do central). Leave unset to disable.
```
...
**Finding your note numbers.** Set `VTT_MIDI_DEBUG=1`, run `./main`, and press the keys you want to use:

```
MIDI: note-on ch=0 note=60 vel=97
```

Put the numbers in `VTT_MIDI_NOTE` and `VTT_MIDI_LANG_NOTE` and turn the debug flag back off.

Other knobs: `VTT_MIDI_CHANNEL` (restrict to one channel, `0`–`15`) and `VTT_MIDI_DEBOUNCE_MS` (default 250, the minimum gap between triggers, shared by both notes).

The listener never brings the daemon down. If the controller is off, unplugged, or held open by a DAW (rawmidi nodes are exclusive-open), it logs the reason once and keeps retrying with backoff; plug the device back in and it reconnects on its own. The window's buttons and the MIDI keys share the same state, so either can undo the other.
